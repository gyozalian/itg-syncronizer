...
public class BankMessageSFTPMover implements MessageMover {
    ...
    public void start() {
        ...
        try {
            SFTPConnector.init(sftpConfig);
        } catch (JSchException e) {
            logger.log(Level.WARNING, "Unable to initilize sftp server");
            return;
        }
    ...
    private void scanAndCopyFiles(boolean fromBank) {
        Path localDir = fromBank ? this.bankDir : Paths.get(this.sftpSboDir);
        String remoteDir = fromBank ? this.sftpSboDir : this.bankDir.toString();

        try {
            if (fromBank) {
                // Local to SFTP
                transferLocalToSftp(localDir, remoteDir);
            } else {
                // SFTP to Local
                transferSftpToLocal(remoteDir, localDir);
            }
    ...
    public void transferLocalToSftp(Path localDir, String remoteDir) throws IOException, JSchException {
        try {
            ChannelSftp sftpClient = SFTPConnector.connect(sftpConfig);
            if (sftpClient == null) {
                logger.log(Level.WARNING, "Unable to connect to SFTP server");
                return;
            }
                ...
                public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                    ...
                    if (shouldTransferFile(attrs) && ALLOWED_FILE_EXTENSIONS.contains(fileExtension)) {
                        ...
                        try {
                            ...
                            sftpClient.put(localDir.normalize().toString(), remoteFilePath);
    ...
    private void transferSftpToLocal(String remoteDir, Path localDir) throws
            IOException, SftpException, JSchException {
        try {
            ChannelSftp channelSftp = SFTPConnector.connect(sftpConfig);
            if (channelSftp == null) {
                logger.log(Level.WARNING, "Unable to connect to SFTP server");
                return;
            }
            transferSftpToLocal(channelSftp, remoteDir, localDir);
        } finally {
            SFTPConnector.disconnect();
        }
    ...
    private void transferSftpToLocal(ChannelSftp channelSftp, String remoteDir, Path localDir) throws
            IOException, SftpException, JSchException {
    ...
    private boolean shouldTransferFile(ChannelSftp.LsEntry entry) {
        int modTime = entry.getAttrs().getMTime(); // Get modification time in seconds
    ...
    private static void ensureDirectoryExists(ChannelSftp channelSftp, String directoryPath) throws SftpException {
